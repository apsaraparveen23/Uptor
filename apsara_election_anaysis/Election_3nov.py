...
# Configuration
# ==============================
manual_csv_path = 'manual_tweets1.csv.csv'
...
def load_mock_data():
    ...
    mock_data = [
        {'tweet_id': '1001', 'time': current_time,
         'tweet': "PM Modi's rally in Meerut drew massive crowds.", 'retweet_count': 1200},
        {'tweet_id': '1002', 'time': current_time,
         'tweet': "Rahul Gandhi focused on youth employment in his Ramlila speech.", 'retweet_count': 850},
        {'tweet_id': '1003', 'time': current_time,
         'tweet': "Election Commission announces new voting guidelines.", 'retweet_count': 430},
        {'tweet_id': '1004', 'time': current_time,
         'tweet': "BJP campaigns in Varanasi with promises of development.", 'retweet_count': 670},
        {'tweet_id': '1005', 'time': current_time,
         'tweet': "Congress accuses BJP of misusing government machinery.", 'retweet_count': 520}
    ]
...
def run_ml_prediction(df):
    ...
    # ---- Apply stronger thresholds for polarity ----
    df['sentiment'] = df['tweet'].apply(lambda x: get_sentiment(x))
    df.loc[df['polarity'] > 0.15, 'sentiment'] = 'positive'
    df.loc[df['polarity'] < -0.15, 'sentiment'] = 'negative'

    print("\n🔍 Polarity check sample:")
    print(df[['tweet', 'polarity', 'sentiment']].head(10))
...
# ==============================
def run_pipeline():
    cleaned_path = '../New_afsara/cleaned_df.csv'
